...
export const TableDetailsPanel: React.FC<TableDetailsPanelProps> = ({ 
  ...
  useEffect(() => {
    if (table) {
      console.log('TableDetailsPanel received table:', table);
      console.log('Table columns:', table.columns);
      if (table.columns) {
        console.log('First column details:', table.columns[0]);
      }
    }
  }, [table]);
      ...
      >
        {table && (
          <>
            ...
            <Divider />
            <Box sx={{ flexGrow: 1, overflow: 'auto', position: 'relative', p: 2 }}>
              {isLoading ? (
